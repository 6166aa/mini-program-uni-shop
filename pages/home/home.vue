<template>
  <view>
    <!-- 自定义导航条 -->
    <nav-bar  leftText="黑马精选"  background="#c00000" color="#fff">
      <view class="search">
    ...
    <!-- 功能区 -->
    <view class="actions">
      <view class="action" v-for="(item, index) in actionData" @click="actionClick(item)" :key="index">
          <image class="icon" :src="item.image_src" mode="widthFix"></image>
      </view>
    </view>
    <!-- 其他分类入口 -->
    <view class="sections">
      <view class="section" v-for="(item, index) in floorData" :key="index" @click="sectionClick">
        ...
        <view class="content">
          <view class="left">
            <image  :data-query="item.product_list[0].navigator_url.split('?')[1]" :src="item.product_list[0].image_src" :style="{ width: `${item.product_list[0].image_width}rpx` }" mode="widthFix"></image>
          </view>
          <view class="right">
